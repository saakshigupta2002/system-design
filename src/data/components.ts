...
export const SYSTEM_COMPONENTS: SystemComponent[] = [
  ...
  // ── AI building blocks (generic, for RAG / agents / LLM apps) ──
  { id: "llm", label: "LLM", category: "ai", icon: "Brain", maxQPS: 500, latencyMs: 1500, scalable: true, stateful: false, monthlyCost: 0, description: "A large language model used for generation and reasoning, via API or self-hosted. High latency (often seconds) and rate-limited — stream tokens, call it asynchronously, and cache where you can." },
  { id: "embedding-model", label: "Embedding Model", category: "ai", icon: "Sparkles", maxQPS: 5000, latencyMs: 80, scalable: true, stateful: false, monthlyCost: 0, description: "Turns text (or code/images) into vector embeddings for semantic search and RAG. Used at ingestion (embed documents) and at query time (embed the question)." },
  { id: "agent-orchestrator", label: "Agent Orchestrator", category: "ai", icon: "Compass", maxQPS: 5000, latencyMs: 50, scalable: true, stateful: true, monthlyCost: 30, description: "Runs the agent loop: ask the LLM what to do next, call tools, observe the result, repeat until the task is done. Manages planning, tool routing, and memory." },
  { id: "mcp-server", label: "MCP Server", category: "ai", icon: "Share2", maxQPS: 10000, latencyMs: 30, scalable: true, stateful: false, monthlyCost: 20, description: "A Model Context Protocol server that exposes tools and data sources to an LLM/agent through a standard interface (search, database, file, or external API access)." },
  { id: "model-server", label: "Model Server (GPU)", category: "ai", icon: "Cpu", maxQPS: 2000, latencyMs: 200, scalable: true, stateful: false, monthlyCost: 300, description: "Self-hosted inference server running models on GPUs with request batching and a KV cache for throughput. Autoscale by queue depth / GPU utilization; expensive, so batch aggressively." },
  { id: "guardrails", label: "Guardrails / Safety", category: "ai", icon: "ShieldCheck", maxQPS: 20000, latencyMs: 30, scalable: true, stateful: false, monthlyCost: 15, description: "Safety layer that screens inputs and outputs: moderation, PII redaction, prompt-injection / jailbreak detection, and policy checks before content reaches the model or the user." },
];
